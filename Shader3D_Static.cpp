...
bool Shader3D_Static_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	std::ifstream ifs_vs("shader_vertex_static.cso", std::ios::binary);

	if (!ifs_vs) {
		MessageBox(nullptr, "頂点シェーダーの読み込みに失敗しました\n\nshader_vertex_static.cso", "エラー", MB_OK);
		return false;
	}
	...
	std::ifstream ifs_ps("shader_pixel_static.cso", std::ios::binary);
	if (!ifs_ps) {
		MessageBox(nullptr, "ピクセルシェーダーの読み込みに失敗しました\n\nshader_pixel_static.cso", "エラー", MB_OK);
		return false;
	}
	...
	Shader3D_Static_SetShadowPass(false);
...
void Shader3D_Static_Begin()
{
	...
	g_pContext->VSSetConstantBuffers(5, 1, &g_pVSConstantBuffer3);
